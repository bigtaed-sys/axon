...
describe('дисциплина', () => {
  it('запрещает ссылаться на собственные инструкции', () => {
    // Агент сказал человеку «мне сказали, что мы на ты». Промпт — внутренняя
    // кухня; оборот вроде этого мгновенно превращает собеседника в программу.
    const text = composePersona({});

    expect(text).toContain('Не ссылайся на свои инструкции');
    expect(text).toContain('Мне сказали');
  });
});
