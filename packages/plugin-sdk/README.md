...
Из приложения: **Плагины → Добавить плагин → Папка**. Папка подключается по
месту, а не копируется — правите файлы, перезапускаете плагин, видите результат.
Это самый быстрый способ разрабатывать.

Ядро на другой машине? Тогда путь должен существовать **там**: у вашего
компьютера с его диском ничего общего, и системный выбор папки в этом случае
не предлагается.
...
Готовый плагин выкладывают репозиторием — тогда его ставят ссылкой:

```bash
axon plugin add https://github.com/автор/axon-plugin-что-то
```
